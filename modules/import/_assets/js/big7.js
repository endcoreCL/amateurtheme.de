	...
	jQuery('.start-import').bind('click', function (e) {
		...
		if (max_videos != "0") {
			...
			jQuery(wrapper).find('tbody tr input[type="checkbox"]:checked').each(function () {
				...
				var xhr = jQuery.post(ajaxurl + '?action=at_big7_import_video', video).done(function (data) {
					if (data != "error") {
...
var at_crawl_amateurs = function (next_offset) {
	...
	jQuery.ajaxQueue({
		...
		dataType: 'json',
		type    : 'GET',
		data    : "action=at_import_big7_crawl_amateurs&offset=" + next_offset,
...
var at_get_videos_html = function (item) {
	...
	html += '<td class="categories">' + item.categories + '</td>';
