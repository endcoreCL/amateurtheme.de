...
function acfgfs_google_font_enqueue(){
    ...
    foreach( $fonts as $font ) {
        // skip open sans, it comes already with the theme
        if($font['font'] == 'Open Sans') {
            continue;
        }
        ...
        if( $font['variants'] == array( 'regular' ) ) {
            ...
            $font_element[] = $font_name . ':' . $font['variants'];
        }
    }
...
function acfgfs_display_variant_list( $field, $new_font = null ) {
    ...
    $i = 1;
    ?><select id="<?php echo $field['key'] ?>_variants" name="<?php echo $field['key'] ?>_variants"><?php
    foreach( $font['variants'] as $variant ) :
        if( empty( $new_font ) ) {
            $checked = ( empty( $field['value'] ) || ( !empty( $field['value'] ) && $variant == $field['value']['variants'] ) ) ? 'selected' : '';
        }
        else {
            $checked = ( $variant == 'regular' ) ? 'selected' : '';
        }

        ?>

        <option value="<?php echo $variant ?>"  <?php echo $checked ?>><?php echo $variant ?></option>
